...
def BOPS(args, domain, Nums, Lists, Datas, target_users, target_items, competing_items, budget):
    '''
    if len(domain[i][1]) > 1:
        assume only 1 target item, more than 1 indicates proxy items for fake user
        than peruser ==> injection attack, budget is for each fake user (domain[i][0])
    else:
        comprehensive attack, budget is total selection out of real user pool
    '''
