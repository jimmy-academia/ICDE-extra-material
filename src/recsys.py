...
from networks.graphrec import GraphRec
...
def prepare_models():
    args = default_args()
    print('train both models on all three datasets!')
    if not os.path.exists(args.cache_dir/'model_weights'):
        os.makedirs(args.cache_dir/'model_weights')
    for dataset in ['epinions', 'ciao', 'library']:
        Nums, Lists, Datas = _readfile(args.cache_dir/(dataset+'.pkl'))
        train_loader, valid_loader, __ = _make_loaders(Datas, 256)
        for Rec, name in zip([GraphRec, GraphConsis], ['graph', 'consis']):
            args.model = name
            filepath = args.cache_dir/'model_weights'/(dataset+'_'+name+'.pth')
            if not os.path.exists(filepath):
                print('training...', filepath)
                model = Rec(args, Nums, deepcopy(Lists)).to(args.device)
...
def execute_recsys(args, Nums, Lists, Datas, target_users, target_items, competing_items, model_path=None):
    train_loader, valid_loader, __ = _make_loaders(Datas, 256)
    Rec = [GraphConsis, GraphRec][['consis', 'graph'].index(args.model)]
    model = Rec(args, Nums, deepcopy(Lists)).to(args.device)
    model = train(args, model, train_loader, valid_loader)
    model.eval()
