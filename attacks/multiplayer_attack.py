...
def MSOPS(args, Domain_list, Nums, Lists, Datas, target_users, target_items, competing_items, b):
    ...
    pbar = tqdm(range(10), ncols=90, desc='multi')

    load_dir = args.task_dir/'mca'/'load'
    if not os.path.exists(load_dir):
        os.makedirs(load_dir)
    if 'load_rd' in args:
        loadpath = load_dir/'b{}rd{}All_selections.pt'.format(b, args.load_rd)
        if os.path.exists(loadpath):
            All_selections = torch.load(loadpath)
            pbar = tqdm(range(args.load_rd+1, args.load_rd+11), ncols=90, desc='multi')
    ...
    for rd in pbar:
        ...
        # pretrain
        for __ in tqdm(range(4), ncols=88, leave=False, desc='pretrain'):
            sur_model.train()
            ...
            for data in dbar:
                ...
                loss = sur_model.frac_loss(batch_nodes_u, batch_nodes_v, labels_, all_sels)
                loss.backward()
                sur_optimizer.step()
        ...
        if rd%10 in [2,5,7,9]:
            tmp_dir = args.task_dir/'mca'/'rd{}'.format(rd)
            if not os.path.exists(tmp_dir):
                os.makedirs(tmp_dir)
            tmp_poison_path = tmp_dir/'poison{}.pt'.format(b)
            torch.save(varsels, tmp_poison_path)
        loadpath = load_dir/'b{}rd{}All_selections.pt'.format(b, rd)
        torch.save(All_selections, loadpath)
